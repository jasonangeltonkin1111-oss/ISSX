...
public:
   static bool EnsureFolder(const string relative_path)
     {
      if(ISSX_Util::IsEmpty(relative_path))
         return false;
      string parts[];
      int n=StringSplit(relative_path,StringGetCharacter(ISSX_PATH_SEP,0),parts);
      string build="";
      for(int i=0;i<n;i++)
        {
   ...
   static bool EnsureParentFolder(const string relative_file_path)
     {
      int slash=StringFind(relative_file_path,ISSX_PATH_SEP,0);
      int last=-1;
      while(slash>=0)
        {
         last=slash;
         slash=StringFind(relative_file_path,ISSX_PATH_SEP,slash+1);
        }
      if(last<=0)
         return true;
   ...
   static bool WriteText(const string relative_path,const string text)
     {
      if(!EnsureParentFolder(relative_path))
         return false;
      int h=FileOpen(relative_path,FILE_WRITE|FILE_TXT|FILE_COMMON|FILE_ANSI);
      if(h==INVALID_HANDLE)
         return false;
      FileWriteString(h,text,StringLen(text));
      FileFlush(h);
      FileClose(h);
      return true;
     }
   ...
   static bool ReadText(const string relative_path,string &out_text)
     {
      out_text="";
      int h=FileOpen(relative_path,FILE_READ|FILE_TXT|FILE_COMMON|FILE_ANSI);
      if(h==INVALID_HANDLE)
         return false;
      out_text=FileReadString(h,(int)FileSize(h));
      FileClose(h);
      return true;
     }
...
private:
   static string BaseRoot()
     {
      return ISSX_Util::JoinPath(ISSX_DIR_FIRMS,ISSX_DIR_ROOT_NAME);
     }
...
public:
   ...
   static string InternalFile(const string firm_id,const ISSX_StageId stage_id,const string filename)
     {
      return ISSX_Util::JoinPath(InternalDir(firm_id,stage_id),filename);
     }
   ...
   static string ExtractRaw(const string json,const string name)
     {
      ...
      if(p<0)
         return "";
      p+=StringLen(name)+3;
      while(p<StringLen(json) && (StringSubstr(json,p,1)==" " || StringSubstr(json,p,1)=="\t" || StringSubstr(json,p,1)=="\r" || StringSubstr(json,p,1)=="\n"))
         p++;
      if(p>=StringLen(json))
         return "";
      ...
      while(q<StringLen(json))
        {
         string ch=StringSubstr(json,q,1);
         if(ch=="," || ch=="}" || ch=="]")
            break;
         q++;
        }
...
public:
   static ISSX_CompatibilityCheck Evaluate(const ISSX_Manifest &have,
                                           ...
                                           const int max_age_minutes=30)
     {
      ...
      c.sequence_monotonic_sane=(have.sequence_no>0 && header.sequence_no==have.sequence_no);
...
public:
   ...
   static bool HeaderPayloadAgree(const ISSX_StageHeader &header,const string payload_text)
     {
      return (header.payload_length==StringLen(payload_text) &&
   ...
   static bool CandidateTrioCoherent(const ISSX_StageHeader &header,const ISSX_Manifest &manifest,const string payload_text)
     {
      return HeaderManifestAgree(header,manifest) && HeaderPayloadAgree(header,payload_text) &&
             header.magic==ISSX_BINARY_MAGIC &&
...
public:
   ...
   static bool ReadAccepted(const string firm_id,
     ...
     {
      outcome.Reset();
      string header_json,manifest_json,payload_text,record_json;
...
private:
   static bool RotateAcceptedToPrevious(const string hc,const string pc,const string mc,
     ...
     {
      bool ok=true;
      if(ISSX_FileIO::Exists(hc)) ok=ok && ISSX_FileIO::CopyText(hc,hp);
      if(ISSX_FileIO::Exists(pc)) ok=ok && ISSX_FileIO::CopyText(pc,pp);
      if(ISSX_FileIO::Exists(mc)) ok=ok && ISSX_FileIO::CopyText(mc,mp);
      return ok;
     }
...
public:
   static bool WriteCandidate(const string firm_id,
                              ...
                              ISSX_Manifest &manifest)
     {
      ISSX_FileIO::EnsureFolder(ISSX_PersistencePath::FirmRoot(firm_id));
      ISSX_FileIO::EnsureFolder(ISSX_PersistencePath::InternalDir(firm_id,stage_id));
      ...
      bool ok=true;
      ok=ok && ISSX_FileIO::WriteText(ISSX_PersistencePath::InternalFile(firm_id,stage_id,ISSX_BIN_HEADER_CANDIDATE),header_json);
      ok=ok && ISSX_FileIO::WriteText(ISSX_PersistencePath::InternalFile(firm_id,stage_id,ISSX_BIN_PAYLOAD_CANDIDATE),payload_text);
      ok=ok && ISSX_FileIO::WriteText(ISSX_PersistencePath::InternalFile(firm_id,stage_id,ISSX_JSON_MANIFEST_CANDIDATE),ISSX_PersistenceCodec::ManifestToJson(manifest));
      return ok;
     }
   ...
   static bool LoadCandidate(const string firm_id,
     ...
     {
      string h,m,p;
      if(!ISSX_FileIO::ReadText(ISSX_PersistencePath::InternalFile(firm_id,stage_id,ISSX_BIN_HEADER_CANDIDATE),h))
         return false;
      if(!ISSX_FileIO::ReadText(ISSX_PersistencePath::InternalFile(firm_id,stage_id,ISSX_JSON_MANIFEST_CANDIDATE),m))
         return false;
      if(!ISSX_FileIO::ReadText(ISSX_PersistencePath::InternalFile(firm_id,stage_id,ISSX_BIN_PAYLOAD_CANDIDATE),p))
         return false;
   ...
   static bool PromoteCandidate(const string firm_id,
                                ...
                                ISSX_ProjectionOutcome &outcome)
     {
      ...
      string hc=ISSX_PersistencePath::InternalFile(firm_id,stage_id,ISSX_BIN_HEADER_CURRENT);
      string pc=ISSX_PersistencePath::InternalFile(firm_id,stage_id,ISSX_BIN_PAYLOAD_CURRENT);
      string mc=ISSX_PersistencePath::InternalFile(firm_id,stage_id,ISSX_JSON_MANIFEST_CURRENT);
      string hp=ISSX_PersistencePath::InternalFile(firm_id,stage_id,ISSX_BIN_HEADER_PREVIOUS);
      string pp=ISSX_PersistencePath::InternalFile(firm_id,stage_id,ISSX_BIN_PAYLOAD_PREVIOUS);
      string mp=ISSX_PersistencePath::InternalFile(firm_id,stage_id,ISSX_JSON_MANIFEST_PREVIOUS);
      string hcand=ISSX_PersistencePath::InternalFile(firm_id,stage_id,ISSX_BIN_HEADER_CANDIDATE);
      string pcand=ISSX_PersistencePath::InternalFile(firm_id,stage_id,ISSX_BIN_PAYLOAD_CANDIDATE);
      string mcand=ISSX_PersistencePath::InternalFile(firm_id,stage_id,ISSX_JSON_MANIFEST_CANDIDATE);
      ...
      if(ok)
         ISSX_FileIO::WriteText(mc,ISSX_PersistenceCodec::ManifestToJson(manifest));
      outcome.internal_commit_success=ok;

      if(ok)
        {
         ISSX_StageHeader promoted_header;
         promoted_header=verify_header;
         ISSX_HandoffStore::WriteAccepted(firm_id,stage_id,promoted_header,manifest,verify_payload);
        }
   ...
   static bool PromoteLastGoodIfEligible(const string firm_id,
                                         ...
                                         const ISSX_Manifest &manifest)
     {
      ...
      string pc=ISSX_PersistencePath::InternalFile(firm_id,stage_id,ISSX_BIN_PAYLOAD_CURRENT);
      string mc=ISSX_PersistencePath::InternalFile(firm_id,stage_id,ISSX_JSON_MANIFEST_CURRENT);
      string pl=ISSX_PersistencePath::InternalFile(firm_id,stage_id,ISSX_BIN_PAYLOAD_LASTGOOD);
      string ml=ISSX_PersistencePath::InternalFile(firm_id,stage_id,ISSX_JSON_MANIFEST_LASTGOOD);
...
public:
   ...
   static bool ProjectFromAccepted(const string firm_id,
                                   ...
                                   ISSX_ProjectionOutcome &outcome)
     {
      ...
      outcome.last_projection_reason=(outcome.projection_partial_success_flag ? "partial_projection" : "ok");
      return export_ok;
     }
  };
...
public:
   static bool ReadBestAvailable(const string firm_id,
                                 ...
                                 ISSX_UpstreamReadOutcome &outcome)
     {
      ...
      string manifest_lastgood_json,payload_lastgood;
      if(ISSX_FileIO::ReadText(ISSX_PersistencePath::InternalFile(firm_id,stage_id,ISSX_JSON_MANIFEST_LASTGOOD),manifest_lastgood_json) &&
         ISSX_FileIO::ReadText(ISSX_PersistencePath::InternalFile(firm_id,stage_id,ISSX_BIN_PAYLOAD_LASTGOOD),payload_lastgood))
        {
         ...
         if(outcome.manifest.accepted_promotion_verified || outcome.manifest.stage_minimum_ready_flag)
           {
            outcome.found=true;
...
public:
   ...
   static bool IsStale(const ISSX_LockLease &lock_state,const datetime now_ts=0)
     {
      datetime nowv=(now_ts>0 ? now_ts : ISSX_Time::BestScheduleClock());
   ...
   static bool Acquire(const string firm_id,
                       ...
                       ISSX_LockLease &out_lock)
     {
      ...
      existing.Reset();
      bool has_existing=Read(firm_id,existing);
...
public:
   static bool WriteCurrent(const string firm_id,const string payload_text,const ISSX_Manifest &manifest,const bool write_snapshot=false)
     {
      ISSX_FileIO::EnsureFolder(ISSX_PersistencePath::UniverseDir(firm_id));
      bool ok=true;
      ok=ok && ISSX_FileIO::WriteText(ISSX_Util::JoinPath(ISSX_PersistencePath::UniverseDir(firm_id),ISSX_BIN_BROKER_UNIVERSE_CURRENT),payload_text);
      ok=ok && ISSX_FileIO::WriteText(ISSX_Util::JoinPath(ISSX_PersistencePath::UniverseDir(firm_id),ISSX_JSON_BROKER_UNIVERSE_MANIFEST),ISSX_PersistenceCodec::ManifestToJson(manifest));
      if(write_snapshot)
         ok=ok && ISSX_FileIO::WriteText(ISSX_Util::JoinPath(ISSX_PersistencePath::UniverseDir(firm_id),ISSX_JSON_BROKER_UNIVERSE_SNAPSHOT),payload_text);
      return ok;
     }

   static bool RotateCurrentToPrevious(const string firm_id)
     {
      string dir=ISSX_PersistencePath::UniverseDir(firm_id);
      return (!ISSX_FileIO::Exists(ISSX_Util::JoinPath(dir,ISSX_BIN_BROKER_UNIVERSE_CURRENT)) ||
              ISSX_FileIO::CopyText(ISSX_Util::JoinPath(dir,ISSX_BIN_BROKER_UNIVERSE_CURRENT),
                                    ISSX_Util::JoinPath(dir,ISSX_BIN_BROKER_UNIVERSE_PREVIOUS)));
     }
  };
...
public:
   ...
   static bool TouchRegistry(const string firm_id,const string symbol_registry_payload,const string timeframe_index_payload,const string hydration_cursor_payload,const string dirty_set_payload,const string manifest_payload)
     {
      bool ok=true;
   ...
   static bool MarkDirty(ISSX_DirtyShardBatch &batch,const string shard_key,const long minute_id)
     {
      if(ISSX_Util::IsEmpty(shard_key))
         return false;
      if(StringFind(","+batch.shard_keys_csv+",",","+shard_key+",")<0)
        {
   ...
   static bool Flush(const string firm_id,ISSX_DirtyShardBatch &batch)
     {
      if(!batch.flush_required)
         return true;
      bool ok=ISSX_FileIO::WriteText(ISSX_Util::JoinPath(ISSX_PersistencePath::HistoryIndexDir(firm_id),ISSX_BIN_HISTORY_DIRTY_SET),batch.shard_keys_csv);
      if(ok)
         batch.Reset();
      return ok;
     }
...
public:
   static bool ValidateCandidateTrioPresence(const string firm_id,const ISSX_StageId stage_id)
     {
      return ISSX_FileIO::Exists(ISSX_PersistencePath::InternalFile(firm_id,stage_id,ISSX_BIN_HEADER_CANDIDATE)) &&
             ISSX_FileIO::Exists(ISSX_PersistencePath::InternalFile(firm_id,stage_id,ISSX_BIN_PAYLOAD_CANDIDATE)) &&
             ISSX_FileIO::Exists(ISSX_PersistencePath::InternalFile(firm_id,stage_id,ISSX_JSON_MANIFEST_CANDIDATE));
     }
