...
// Compatibility enums required by the large v1.7 engine build.
// These are defined locally here because the current shared core surface
// does not yet provide them, while the original EA4 engine uses them.
enum ISSX_CorrRejectReason
  {
   issx_corr_reject_not_enough_overlap = 0,
   issx_corr_reject_stale_history      = 1,
   issx_corr_reject_low_variation      = 2,
   issx_corr_reject_alignment_fail     = 3,
   issx_corr_reject_budget_skipped     = 4,
   issx_corr_reject_duplicate_preempted= 5
  };

enum ISSX_PenaltyBasisKind
  {
   issx_penalty_family = 0,
   issx_penalty_session,
   issx_penalty_returns,
   issx_penalty_bucket,
   issx_penalty_mixed
  };
...
enum ISSX_PortfolioRoleHint
  {
   issx_role_anchor = 0,
   issx_role_overlap_risk,
   issx_role_diversifier,
   issx_role_fragile_diversifier,
   issx_role_redundant
  };
...
public:
   ...
   static bool StageSlice(ISSX_EA4_State &state,
     ...
     {
      return BuildState(firm_id,ea1,ea2,ea3,state);
     }
