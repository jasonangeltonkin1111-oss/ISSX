...
#define EA_NAME                 "EA1"
#define EA_ENGINE_VERSION       "2.1"
#define EA_SCHEMA_VERSION       "2.4"
#define EA_BUILD_CHANNEL        "lean_final"
   ...
   while(StringLen(out) > 0)
     {
      ...
      if(tail == "." || tail == "_" || tail == " ") out = StringSubstr(out, 0, StringLen(out)-1);
      else break;
     }
...
string BuildFirmId()
  {
   ...
   if(suffix != "" && suffix != "UNKNOWN_FIRM") return firm + "_" + suffix;
   return firm;
  }
...
long ReadSymbolIntegerSafe(const string sym, const ENUM_SYMBOL_INFO_INTEGER prop, const long fallback)
  {
   long v = 0;
   ResetLastError();
   if(SymbolInfoInteger(sym, prop, v)) return v;
   return fallback;
  }
...
double ReadSymbolDoubleSafe(const string sym, const ENUM_SYMBOL_INFO_DOUBLE prop, const double fallback)
  {
   double v = 0.0;
   ResetLastError();
   if(SymbolInfoDouble(sym, prop, v)) return v;
   return fallback;
  }
...
string ReadSymbolStringSafe(const string sym, const ENUM_SYMBOL_INFO_STRING prop, const string fallback)
  {
   string v = "";
   ResetLastError();
   if(SymbolInfoString(sym, prop, v)) return v;
   return fallback;
  }
...
int StableJitterSec(const string key, const int spread_sec)
  {
   if(spread_sec <= 1) return 0;
   return (int)(FNV1a32(key) % (uint)spread_sec);
  }

int SnapshotRetryDelaySec(const SymbolRecord &rec)
  {
   if(rec.tick_state.activity_class == SAC_ACTIVE) return 0;
   int base = MathMax(1, InpSnapshotClosedRecheckSec);
   int spread = MathMax(1, MathMin(30, base / 4));
   return base + StableJitterSec(rec.normalized_symbol, spread);
  }

int CopyTicksRetryDelaySec(const SymbolRecord &rec)
  {
   if(rec.tick_state.activity_class == SAC_ACTIVE) return MathMax(1, InpCopyTicksActiveRecheckSec);
   if(rec.tick_state.activity_class == SAC_MARKET_CLOSED)
     {
      int base = MathMax(1, InpCopyTicksClosedRecheckSec);
      int spread = MathMax(1, MathMin(60, base / 4));
      return base + StableJitterSec(rec.normalized_symbol, spread);
     }
   ...
   int base = has_any_tick_evidence ? MathMax(1, InpCopyTicksDormantRecheckSec) : MathMax(1, InpCopyTicksUnknownRecheckSec);
   int spread = MathMax(1, MathMin(300, base / 8));
   return base + StableJitterSec(rec.normalized_symbol, spread);
  }
...
void UpdateSymbolActivityState(SymbolRecord &rec)
  {
   ...
   else if(rec.tick_state.next_snapshot_due_server <= 0 ||
           prev_activity != rec.tick_state.activity_class ||
           prev_market != rec.tick_state.market_open_state ||
           rec.tick_state.next_snapshot_due_server <= g_schedule.now_server_time)
      rec.tick_state.next_snapshot_due_server = g_schedule.now_server_time + SnapshotRetryDelaySec(rec);

   if(rec.tick_state.activity_class == SAC_ACTIVE)
      rec.tick_state.active_watch_due_server = g_schedule.now_server_time;
   else if(rec.tick_state.activity_class == SAC_MARKET_CLOSED)
     {
      if(rec.tick_state.reopen_watch_due_server <= 0 ||
         prev_activity != SAC_MARKET_CLOSED ||
         prev_market != MOS_CLOSED_NOW ||
         rec.tick_state.reopen_watch_due_server <= g_schedule.now_server_time)
         rec.tick_state.reopen_watch_due_server = g_schedule.now_server_time + CopyTicksRetryDelaySec(rec);
     }
   else
     {
      if(rec.tick_state.dead_watch_due_server <= 0 ||
         prev_activity != SAC_DORMANT ||
         rec.tick_state.dead_watch_due_server <= g_schedule.now_server_time)
         rec.tick_state.dead_watch_due_server = g_schedule.now_server_time + CopyTicksRetryDelaySec(rec);
     }
...
void ReadSnapshotTick(SymbolRecord &rec)
  {
   ...
   else if(has_bid) rec.tick_state.mid = tick.bid;
   else rec.tick_state.mid = tick.ask;
   ...
   for(int k = 0; k < work; k++)
     {
      ...
      if(due) LoadSessions(g_universe.records[idx]);
      g_universe.records[idx].base_dirty = true;
     }
  }
...
void CopyTicksForSymbol(SymbolRecord &rec)
  {
   ...
   ulong from_msc = 0;
   if(rec.tick_state.last_copied_msc > 0) from_msc = (ulong)rec.tick_state.last_copied_msc;
      ...
      if(rec.tick_state.activity_class == SAC_MARKET_CLOSED || rec.tick_state.activity_class == SAC_DORMANT)
        {
         ...
         rec.tick_state.copyticks_phase = (rec.tick_state.ring_count > 0 ? CT_STEADY : CT_NOT_STARTED);
         rec.tick_state.next_copyticks_due_server = g_schedule.now_server_time + CopyTicksRetryDelaySec(rec);
         return;
        }
...
bool DidMaterialContinuityChange(SymbolRecord &rec)
  {
   if(rec.continuity.last_saved_copyticks_phase != (int)rec.tick_state.copyticks_phase) return true;
...
bool SaveSymbolPersistence(SymbolRecord &rec)
  {
   ...
   if(rec.continuity.continuity_origin == CO_NONE) rec.continuity.continuity_origin = CO_CLEAN;
...
bool LoadSymbolPersistence(SymbolRecord &rec)
  {
   ...
   if(!FileIsExist(use_path, FILE_COMMON))
     {
      if(FileIsExist(backup_rel, FILE_COMMON)) use_path = backup_rel;
      else
        {
   ...
   if(schema_ver != EA1_PERSIST_SCHEMA_VERSION ||
      eng_ver != EA_ENGINE_VERSION ||
      bp_ver != EA_SCHEMA_VERSION ||
...
void InitPersistence()
  {
   ...
   for(int i = 0; i < g_universe.size; i++) LoadSymbolPersistence(g_universe.records[i]);
  }
   ...
   for(int k = 0; k < work; k++)
     {
      ...
      if(SaveSymbolPersistence(g_universe.records[idx])) g_persist_writes_this_minute++;
     }
  }
   ...
   for(int i = 0; i < g_universe.size; i++)
     {
      ...
      if(g_universe.records[i].tick_state.market_open_state == MOS_OPEN_NOW) g_market_open_count++;
      else if(g_universe.records[i].tick_state.market_open_state == MOS_CLOSED_NOW) g_market_closed_count++;
      else g_market_unknown_count++;
...
void RenderHUD()
  {
   ...
   text += "Firm: " + g_firm_id + "\n";
   text += "Universe/Selected: " + IntegerToString(g_universe.size) + "/" + IntegerToString(g_selected_count) + "\n";
   text += "Spec/Snap/Hist/Cost: "
        +  IntegerToString(g_spec_ready_count) + "/"
        +  IntegerToString(g_tick_ready_count) + "/"
        +  IntegerToString(g_history_ready_count) + "/"
        +  IntegerToString(g_cost_ready_count) + "\n";
   text += "Open/Closed/Unknown: "
        +  IntegerToString(g_market_open_count) + "/"
        +  IntegerToString(g_market_closed_count) + "/"
        +  IntegerToString(g_market_unknown_count) + "\n";
   text += "Dead Quote: " + IntegerToString(g_dead_quote_count) + "\n";
   text += "Stage/Debug: " + (g_publish_stage.ok ? "1" : "0") + "/" + (g_publish_debug.ok ? "1" : "0") + "\n";
...
string BuildCoverageOnlyJson()
  {
   string s = "{";
   s += "\"selected_count\":" + IntegerToString(g_selected_count) + ",";
   s += "\"ready_spec_count\":" + IntegerToString(g_spec_ready_count) + ",";
   s += "\"session_ready_count\":" + IntegerToString(g_session_ready_count) + ",";
   s += "\"ready_tick_count\":" + IntegerToString(g_tick_ready_count) + ",";
   s += "\"history_ready_count\":" + IntegerToString(g_history_ready_count) + ",";
   s += "\"both_path_ready_count\":" + IntegerToString(g_both_path_ready_count) + ",";
   s += "\"active_symbol_count\":" + IntegerToString(g_active_symbol_count) + ",";
   s += "\"dormant_symbol_count\":" + IntegerToString(g_dormant_symbol_count) + ",";
   s += "\"cost_ready_count\":" + IntegerToString(g_cost_ready_count) + ",";
   s += "\"cost_usable_count\":" + IntegerToString(g_cost_usable_count) + ",";
   s += "\"market_open_count\":" + IntegerToString(g_market_open_count) + ",";
   s += "\"market_closed_count\":" + IntegerToString(g_market_closed_count) + ",";
   s += "\"market_unknown_count\":" + IntegerToString(g_market_unknown_count) + ",";
...
string BuildSymbolJson(const SymbolRecord &rec)
  {
   ...
   s += "\"market\":{";
   s += "\"bid\":" + JsonDoubleOrNull6(rec.tick_state.snapshot_path_ok ? rec.tick_state.bid : INVALID_DBL) + ",";
   s += "\"ask\":" + JsonDoubleOrNull6(rec.tick_state.snapshot_path_ok ? rec.tick_state.ask : INVALID_DBL) + ",";
   s += "\"mid\":" + JsonDoubleOrNull6(rec.tick_state.snapshot_path_ok ? rec.tick_state.mid : INVALID_DBL) + ",";
   s += "\"spread_points\":" + JsonDoubleOrNull6(rec.tick_state.snapshot_path_ok ? rec.tick_state.spread_points : INVALID_DBL);
   s += "},";
...
string BuildEA1MarketJson()
  {
   ...
   out += "\"engine_version\":" + JsonString(EA_ENGINE_VERSION) + ",";
   out += "\"schema_version\":" + JsonString(EA_SCHEMA_VERSION) + ",";
...
string BuildEA1DebugJson()
  {
   ...
   s += "\"producer\":\"EA1\",";
   s += "\"stage\":\"debug\",";
   s += "\"engine_version\":" + JsonString(EA_ENGINE_VERSION) + ",";
   s += "\"schema_version\":" + JsonString(EA_SCHEMA_VERSION) + ",";
...
void MaybePublishStage()
  {
   if(!InpPublishStageEnabled || g_schedule.now_server_time <= 0) return;
...
void MaybePublishDebugJson()
  {
   if(!InpPublishDebugJsonEnabled || g_schedule.now_server_time <= 0) return;
